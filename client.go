...
func NewV5Client(key string) *V4Client {
	c := &V5Client{}
	c.ApiKey = key
...
func (c *V5Client) request(timeout time.Duration, params map[string]string) (PiplResponse, error) {
	req, _ := url.Parse(c.BaseURL)
	p := url.Values{}
...
func (c *V5Client) SearchPerson(p Person, timeout time.Duration, params map[string]string) (PiplResponse, error) {
	person, err := json.Marshal(p)
	if err != nil {
		return PiplResponse{}, err
	}
...
func (c *V5Client) SearchPointer(search_pointer string, timeout time.Duration, params map[string]string) (PiplResponse, error) {
	params["search_pointer"] = search_pointer
	return c.request(timeout, params)
}
...
func (c *V5Client) SearchParams(search_pointer string, timeout time.Duration, params map[string]string) (PiplResponse, error) {
	return c.request(timeout, params)
}
